...
export function ParticleField({
  ...
  useEffect(() => {
    ...
    let currentImage: HTMLImageElement | HTMLCanvasElement | null = null;
    let loadToken = 0;
    ...
    const sampleTargets = (image: HTMLImageElement | HTMLCanvasElement): ParticleTarget[] => {
    ...
    const buildFresh = (image: HTMLImageElement | HTMLCanvasElement) => {
      if (!image.width || !image.height) return;
      ensureCanvasSize();
    ...
    // instead of snapping.
    const morphTo = (image: HTMLImageElement | HTMLCanvasElement) => {
      if (!image.width || !image.height) return;
      ...
      // Surplus particles just fade in place
      for (let k = matched; k < n; k++) {
        particles[pOrder[k]].fading = true;
      }

      // Extras: spawn new particles on a small random ring
    ...
    const loadAndApply = (nextSrc: string, asMorph: boolean) => {
      ...
      if (nextSrc === "orbit") {
        ...
        if (o) {
          ...
          o.translate(cx, cy);
          o.rotate(-0.35);
          o.lineWidth = Math.max(2, R * 0.16);
          o.strokeStyle = "#fff";
          o.beginPath();
          o.ellipse(0, 0, R * 1.9, R * 0.55, 0, 0, Math.PI * 2);
          o.stroke();
          o.restore();
          
          o.beginPath();
          o.arc(cx + R * 2.5, cy - R * 1.35, R * 0.22, 0, Math.PI * 2);
          o.fill();
          
          o.lineWidth = 1;
          o.strokeStyle = "rgba(255, 255, 255, 0.5)";
          o.beginPath();
          o.ellipse(cx, cy, R * 2.5, R * 1.45, -0.2, 0, Math.PI * 2);
          o.stroke();
        }
        currentImage = off;
        if (asMorph) morphTo(off);
        else buildFresh(off);
        return;
      }
