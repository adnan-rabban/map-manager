...
class App {
    ...
    private exportGroup(group: Group): void {
    ...
    private showExportOptionsModal(group: Group, locations: Location[]): void {
        const content = document.createElement('div');
        content.style.marginTop = '16px';
        content.innerHTML = `
            <div class="form-group">
        ...
        this.createModal(`Export ${group.name}`, content, () => {
             const selected = content.querySelector('input[name="export-format"]:checked') as HTMLInputElement;
             const format = selected ? selected.value : 'json';
             this.performExport(group, locations, format);
        }, 'Export');
    }

    private async performExport(group: Group, locations: Location[], format: string): Promise<void> {
